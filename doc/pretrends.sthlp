...
{pstd}
At least one of {opt numpre()} or {opt pre()} and {opt post()} are required options to indicate which portion of the vector correspond to the pre and post periods; at least one of {opt power()}, {opt slope()}, or {opt delta()} must be specified as well. In addition, power calculations can be done sepparately (computes the slope of a linear trend for which pre-trends tests have a given power level):
...
{phang2}{cmd:. pretrends, numpre(3) b(`beta') v(`sigma') power(0.5)}{p_end}
